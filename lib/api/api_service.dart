...
class ApiService {
  ...
  static Future<List<Actor>?> getTrendActor(String period) async {
    ...
    try {
      http.Response response = await http.get(Uri.parse(
          '${Api.baseUrl}${ApiEndPoints.trendingActorsPath}/$period?api_key=${Api.apiKey}&language=en-US'));
      var res = jsonDecode(response.body);
      if (res['results'] != null) {
        res['results'].forEach(
          (a) => actors.add(
  ...
  static Future<Actor?> getActorDetails(int actorId) async {
    try {
      http.Response detailsResponse = await http.get(Uri.parse(
          '${Api.baseUrl}${ApiEndPoints.individualActorDetailsPath}$actorId?api_key=${Api.apiKey}&language=en-US'));
      var detailsData = jsonDecode(detailsResponse.body);

      http.Response filmographyResponse = await http.get(Uri.parse(
          '${Api.baseUrl}${ApiEndPoints.actorMovieCreditsPath.replaceFirst('{actor_id}', actorId.toString())}?api_key=${Api.apiKey}&language=en-US'));
  ...
  static Future<List<Actor>?> getPopularActors() async {
    ...
    try {
      http.Response response = await http.get(Uri.parse(
          '${Api.baseUrl}${ApiEndPoints.popularActorsPath}?api_key=${Api.apiKey}&language=en-US&page=1'));
      var res = jsonDecode(response.body);
      res['results'].forEach(
